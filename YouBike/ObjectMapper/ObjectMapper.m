...
+ (id) mapper {
  return [[ObjectMapper alloc] init];
}
  ...
  for (NSString *key in [source allKeys]) {
    ...
    switch ([self jsonType:value]) {
      case Object:
        ...
        targetPropertyValue = [[targetPropertyClazz alloc] init];
...
- (NSError*)parsingErrorWithDescription:(NSString*)format, ... {   
  ...
  NSDictionary *errorInfo = [NSDictionary dictionaryWithObjectsAndKeys: 
                             ...
                             nil];
  return [NSError errorWithDomain:@"com.wadpam.ObjectMapper.ErrorDomain" code:1 userInfo:errorInfo];
}
